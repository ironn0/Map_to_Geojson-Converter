...
export function setTool(tool) {
    ...
    const activeBtn = document.getElementById(toolButtonIds[tool] || 'tool-' + tool);
    if (activeBtn) {
    ...
    if (prevTool !== tool) {
        const toolNames = {
            ...
            'move': 'Sposta - trascina la forma',
            'scale': 'Scala - trascina la forma verso l’alto o il basso',
            'erase': 'Correzione - marca le parti da ignorare e risegmenta'
        };
        if (toolNames[tool]) {
    ...
    if (el.polygonEditor) {
        el.polygonEditor.classList.toggle('eraser-active', tool === 'erase');
    }
    if (el.imageEditPanel) {
        el.imageEditPanel.classList.toggle('hidden', tool !== 'erase');
    }
    document.body.classList.toggle('image-edit-mode', tool === 'erase');
...
export function startScaleShape(regionIdx, startEvent, renderPolygons) {
    ...
    const eventToImage = e => {
        const bounds = document.getElementById('main-canvas')?.getBoundingClientRect();
        if (!bounds) return [cx, cy];
        return [
            (e.clientX - bounds.left) / state.canvasScale,
            (e.clientY - bounds.top) / state.canvasScale
        ];
    };
    const startPoint = eventToImage(startEvent);
    const startDistance = Math.max(1, Math.hypot(startPoint[0] - cx, startPoint[1] - cy));
    
    const onMove = e => {
        const currentPoint = eventToImage(e);
        const currentDistance = Math.max(1, Math.hypot(currentPoint[0] - cx, currentPoint[1] - cy));
        const radialScale = currentDistance / startDistance;
        const verticalScale = 1 + (startY - e.clientY) * 0.005;
        const scale = clamp(startDistance > 8 ? radialScale : verticalScale, 0.15, 5);
